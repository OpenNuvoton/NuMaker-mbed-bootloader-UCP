        ...
        if (header_status.error == ERR_NONE) {
            ...
            if (result.error == ERR_NONE) {
                ...
                if (erase_size <= slot_size) {
                    /* NOTE: Refer to MBEDBL_UCP_PrepareWrite(...) for why divide underlying
         *                   Prepare(...) into two modes: dummy hash mode and valid hash mode */
                    int dummy_hash_mode = (details->campaign[0] == 0x00);

                    /* erase */
                    int status = dummy_hash_mode ? arm_uc_blockdevice_erase(slot_addr, erase_size) : ARM_UC_BLOCKDEVICE_SUCCESS;
                    ...
                    if (status == ARM_UC_BLOCKDEVICE_SUCCESS) {
                        ...
                        /* write header */
                        status = !dummy_hash_mode ? arm_uc_blockdevice_program(buffer->ptr,
                                                            slot_addr,
                                                            pal_blockdevice_hdr_size) : ARM_UC_BLOCKDEVICE_SUCCESS;
